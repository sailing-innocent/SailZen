...
class HistoryEventDAO(BaseDAO[HistoryEvent]):
    ...
    def get_by_parent(self, parent_event_id: int) -> List[HistoryEvent]:
        ...
        return (
            ...
            .filter(HistoryEvent.parent_event == parent_event_id)
            .order_by(HistoryEvent.start_time)
            .all()
        )
    ...
    def get_by_tag(self, tag: str) -> List[HistoryEvent]:
        ...
        return (
            self.db.query(HistoryEvent)
            .filter(HistoryEvent.tags.contains([tag]))
            .order_by(HistoryEvent.start_time.desc())
            .all()
        )
    ...
    def get_root_events(self) -> List[HistoryEvent]:
        ...
        return (
            self.db.query(HistoryEvent)
            .filter(HistoryEvent.parent_event.is_(None))
            .order_by(HistoryEvent.start_time.desc())
            .all()
        )
