...
class ProjectDAO(BaseDAO[Project]):
    ...
    def get_active_projects(self) -> List[Project]:
        ...
        return (
            ...
            .filter(Project.state.in_([0, 1, 2, 3, 4, 5]))
            .order_by(Project.ctime.desc())
            .all()
        )
    ...
    def get_projects_by_time_range(self, start_qbw: int, end_qbw: int) -> List[Project]:
        """获取指定时间范围内的项目
        ...
        return (
            ...
            .order_by(Project.start_time_qbw)
            .all()
        )
...
class MissionDAO(BaseDAO[Mission]):
    ...
    def get_by_parent(self, parent_id: int) -> List[Mission]:
        ...
        return (
            self.db.query(Mission)
            .filter(Mission.parent_id == parent_id)
            .order_by(Mission.lft)
            .all()
        )
    ...
    def get_root_missions(self) -> List[Mission]:
        ...
        return (
            self.db.query(Mission)
            .filter(Mission.parent_id.is_(None))
            .order_by(Mission.ctime.desc())
            .all()
        )
    ...
    def get_pending_missions(self) -> List[Mission]:
        ...
        return (
            self.db.query(Mission)
            .filter(Mission.state == 0)
            .order_by(Mission.ddl)
            .all()
        )
    ...
    def get_by_tree_id(self, tree_id: int) -> List[Mission]:
        ...
        return (
            self.db.query(Mission)
            .filter(Mission.tree_id == tree_id)
            .order_by(Mission.lft)
            .all()
        )
