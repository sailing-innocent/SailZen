...
class WeightDAO(BaseDAO[Weight]):
    ...
    def get_by_tag(self, tag: str) -> List[Weight]:
        """通过标签获取体重记录"""
        return self.db.query(Weight).filter(Weight.tag == tag).order_by(Weight.htime.desc()).all()
...
class BodySizeDAO(BaseDAO[BodySize]):
    ...
    def get_by_tag(self, tag: str) -> List[BodySize]:
        """通过标签获取身体尺寸记录"""
        return self.db.query(BodySize).filter(BodySize.tag == tag).order_by(BodySize.htime.desc()).all()
...
class ExerciseDAO(BaseDAO[Exercise]):
    ...
    def get_latest(self, limit: int = 10) -> List[Exercise]:
        """获取最新运动记录"""
        return self.db.query(Exercise).order_by(Exercise.htime.desc()).limit(limit).all()
...
class WeightPlanDAO(BaseDAO[WeightPlan]):
    ...
    def get_active_plans(self) -> List[WeightPlan]:
        """获取所有活跃计划（未过期的）"""
        return self.db.query(WeightPlan).filter(
            WeightPlan.target_time >= func.current_timestamp()
        ).order_by(WeightPlan.target_time).all()
