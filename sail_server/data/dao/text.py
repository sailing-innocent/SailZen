...
class WorkDAO(BaseDAO[Work]):
    ...
    def search_by_title(self, title_pattern: str) -> List[Work]:
        ...
        return self.db.query(Work).filter(
            Work.title.ilike(f"%{title_pattern}%")
        ).all()
...
class EditionDAO(BaseDAO[Edition]):
    ...
    def get_by_work(self, work_id: int) -> List[Edition]:
        """获取作品的所有版本"""
        return self.db.query(Edition).filter(
    ...
    def get_by_source_checksum(self, checksum: str) -> Optional[Edition]:
        """通过源文件校验和获取版本"""
        return self.db.query(Edition).filter(
            Edition.source_checksum == checksum
        ).first()
...
class DocumentNodeDAO(BaseDAO[DocumentNode]):
    ...
    def get_by_edition(self, edition_id: int) -> List[DocumentNode]:
        """获取版本的所有文档节点"""
        return self.db.query(DocumentNode).filter(
...
class IngestJobDAO(BaseDAO[IngestJob]):
    ...
    def __init__(self, db: Session):
        super().__init__(db, IngestJob)
    
    def get_by_edition(self, edition_id: int) -> List[IngestJob]:
        """获取版本的所有导入作业"""
        return self.db.query(IngestJob).filter(
            IngestJob.edition_id == edition_id
        ).order_by(IngestJob.created_at.desc()).all()
    
    def get_by_status(self, status: str) -> List[IngestJob]:
        """获取指定状态的所有导入作业"""
        return self.db.query(IngestJob).filter(IngestJob.status == status).all()
    
    def get_pending_jobs(self) -> List[IngestJob]:
        """获取所有待处理的导入作业"""
        return self.db.query(IngestJob).filter(IngestJob.status == "pending").all()
    
    def get_running_jobs(self) -> List[IngestJob]:
        """获取所有运行中的导入作业"""
        return self.db.query(IngestJob).filter(IngestJob.status == "running").all()
