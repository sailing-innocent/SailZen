...
class AnalysisTaskRunner:
    ...
    def create_execution_plan(
        self, db: Session, task_id: int, mode: TaskExecutionMode
    ) -> TaskExecutionPlan:
        """创建任务执行计划"""
    ...
    def _prepare_chapter_chunks(
        self, db: Session, task: AnalysisTask
    ) -> List[ChapterChunk]:
        """准备章节内容并分块"""
        chunks = []
        ...
        if task.target_node_ids and len(task.target_node_ids) > 0:
            nodes = (
                ...
                .filter(DocumentNode.id.in_(task.target_node_ids))
    ...
    async def run_task(
        ...
        try:
            ...
            logger.info(
                f"[DEBUG] Task info: type={task.task_type}, edition_id={task.edition_id}"
            )
            ...
            logger.info(
                f"[DEBUG] Execution plan created: {len(plan.chunks)} chunks, template={plan.prompt_template_id}"
            )
            ...
            for i, chunk in enumerate(plan.chunks):
                ...
                logger.info(
                    f"[DEBUG] Processing chunk {i + 1}/{len(plan.chunks)}: {chunk.chapter_range} "
                ...
                chunk_process_time = (
                    datetime.utcnow() - chunk_process_start
                ).total_seconds()
                logger.info(
                    f"[DEBUG] Chunk {i + 1} processed in {chunk_process_time:.2f}s, "
    ...
    async def _process_chunk_with_llm(
        self, db: Session, task: AnalysisTask, chunk: ChapterChunk, template_id: str
    ) -> List[Dict[str, Any]]:
        """使用 LLM 处理分块"""
        logger.info(
            f"[DEBUG] Starting LLM processing for chunk {chunk.index}: {chunk.chapter_range}"
        )
        ...
        variables = {
            ...
            "setting_types": task.parameters.get(
        ...
        logger.info(
            f"[DEBUG] Template rendered: {template_id}, estimated_tokens: {rendered.estimated_tokens}"
        )

        # 调用 LLM
        client = self._get_llm_client()
        logger.info(
            f"[DEBUG] LLM client created, starting API call for chunk {chunk.index}..."
        )
        ...
        try:
            ...
            response = await client.complete(
                rendered.user_prompt, system=rendered.system_prompt
            )

            api_duration = time.time() - api_start
            logger.info(
                f"[DEBUG] LLM API call completed for chunk {chunk.index}: "
            ...
            if response.finish_reason == "length":
                logger.warning(
                    f"LLM response truncated due to max_tokens limit for chunk {chunk.index}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"JSON parse error for chunk {chunk.index}: {e}, attempting to fix..."
                )
                # 尝试修复不完整的 JSON
    ...
    async def _generate_chunk_prompt(
        ...
        exported = client.generate_prompt_only(
            ...
            task_id=task.id,
            chunk_index=chunk.index,
            total_chunks=self._progress.get(
                task.id,
                TaskProgress(
                    task_id=task.id,
                    status="",
    ...
    def _parse_llm_output(
        self, task_type: str, parsed: Dict[str, Any], chunk: ChapterChunk
    ) -> List[Dict[str, Any]]:
        """解析 LLM 输出为结果列表"""
        results = []
    ...
    def subscribe_progress(
        self, task_id: int, callback: Callable[[TaskProgress], None]
    ):
        """订阅任务进度更新"""
...
def import_external_result(
    db: Session, task_id: int, chunk_index: int, result_text: str
