...
from sail.llm.gateway import create_default_gateway
...
class UnifiedSchedulerWithWebSocket(UnifiedAgentScheduler):
    ...
    def _on_progress_event(self, event: TaskProgressEvent):
        ...
        try:
            ...
            if event.event_type == "started":
                asyncio.create_task(
                    self._ws_manager.notify_task_started(event.task_id, event.data)
                )

            elif event.event_type == "progress":
                asyncio.create_task(
                    self._ws_manager.notify_task_progress(
                        event.task_id,
                ...
                asyncio.create_task(
                    self._ws_manager.notify_task_step(
                        event.task_id,
                ...
                asyncio.create_task(
                    self._ws_manager.notify_task_completed(event.task_id, event.data)
                )

            elif event.event_type == "failed":
                asyncio.create_task(
                    self._ws_manager.notify_task_failed(
                        event.task_id,
                ...
                asyncio.create_task(
                    self._ws_manager.notify_task_cancelled(
                        event.task_id, event.data.get("reason", "user_request")
                    )
                )
    ...
    async def _execute_task(self, task_id: int):
        ...
        """
        logger.info(
            f"[UnifiedSchedulerWithWebSocket] Executing task: task_id={task_id}"
        )
        ...
        with self.db_factory() as db:
            ...
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Task {task_id} found: type={task.task_type}"
            )

            # 创建开始步骤
            step_number = step_dao.get_next_step_number(task_id)
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Creating step {step_number} for task {task_id}"
            )
            ...
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Step created: step_id={step.id}"
            )
            ...
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Event created: event_id={event.id}"
            )

            # 更新进度
            dao.update_progress(task_id, 10, "executing")
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Task {task_id} progress updated to 10%"
            )
            ...
            if not agent:
                logger.error(
                    f"[UnifiedSchedulerWithWebSocket] No agent found for task type: {task.task_type}"
                )
                raise ValueError(f"No agent found for task type: {task.task_type}")

            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Using agent: {agent.agent_type}"
            )
            ...
            def progress_callback(update: ProgressUpdate):
                logger.info(
                    f"[UnifiedSchedulerWithWebSocket] Task {task_id} progress: {update.progress}% - {update.phase} - {update.message}"
                )
            ...
            # 调用 Agent 执行
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Calling agent.execute for task {task_id}"
            )
            result = await agent.execute(task, context, progress_callback)

            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Agent execution result: success={result.success}"
            )
            ...
            if result.success:
                ...
                step_number += 1
                step_content = (
                    result.result_data.get("response", "Task completed")
                ...
                logger.info(
                    f"[UnifiedSchedulerWithWebSocket] Task {task_id} marked as completed"
                )
                ...
                # 执行失败
                logger.error(
                    f"[UnifiedSchedulerWithWebSocket] Task {task_id} failed: {result.error_message}"
                )
                ...
                # 标记失败
                dao.mark_as_failed(
                    task_id, result.error_message or "Unknown error", result.error_code
                )
            ...
            logger.info(
                f"[UnifiedSchedulerWithWebSocket] Task {task_id} execution completed"
            )
...
def get_unified_scheduler_with_ws(
    ...
    if _scheduler_ws_instance is None:
        ...
        _scheduler_ws_instance = UnifiedSchedulerWithWebSocket(
            db_session_factory, config, ws_manager
        )
