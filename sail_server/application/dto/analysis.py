...
class TextRangeContent(BaseModel):
    ...
    chapters: List[Dict[str, Any]] = Field(
        default_factory=list, description="章节内容列表"
    )
...
class CharacterResponse(CharacterBase):
    ...
    first_appearance_node_id: Optional[int] = Field(
        default=None, description="首次出现节点ID"
    )
...
class ExtractedOutlineNode(BaseModel):
    ...
    significance: str = Field(
        default="", description="重要性 (critical/major/normal/minor)"
    )
    sort_index: int = Field(default=0, description="排序索引")
    ...
    evidence_list: List[OutlineEvidence] = Field(
        default_factory=list, description="文本证据列表"
    )
    ...
    def has_full_evidence(self) -> bool:
        ...
        return any(
            len(evidence.text) > 200 for evidence in self.evidence_list if evidence.text
        )

    def get_truncated_evidence_list(
        self, max_length: int = 200
    ) -> List[OutlineEvidence]:
...
class OutlineExtractionConfig(BaseModel):
    ...
    outline_type: str = Field(
        default="main", description="大纲类型 (main/subplot/character_arc/theme)"
    )
...
class OutlineExtractionRequest(BaseModel):
    ...
    edition_id: int = Field(description="版本ID")
    range_selection: TextRangeSelection = Field(description="文本范围选择")
    config: OutlineExtractionConfig = Field(
        default_factory=OutlineExtractionConfig, description="提取配置"
    )
    work_title: str = Field(default="", description="作品标题")
    known_characters: List[str] = Field(
        default_factory=list, description="已知人物列表"
    )
...
class OutlineExtractionResult(BaseModel):
    ...
    nodes: List[ExtractedOutlineNode] = Field(
        default_factory=list, description="提取的节点列表"
    )
    turning_points: List[Dict[str, Any]] = Field(
        default_factory=list, description="转折点列表"
    )
...
class OutlineExtractionResponse(BaseModel):
    ...
    result: Optional[OutlineExtractionResult] = Field(
        default=None, description="提取结果"
    )
...
class DetectedCharacter(BaseModel):
    ...
    aliases: List[DetectedCharacterAlias] = Field(
        default_factory=list, description="别名列表"
    )
    role_type: str = Field(default="supporting", description="角色类型")
    role_confidence: float = Field(default=0.5, description="角色置信度")
    first_appearance: Optional[Dict[str, str]] = Field(
        default=None, description="首次出现"
    )
    description: str = Field(default="", description="描述")
    attributes: List[DetectedCharacterAttribute] = Field(
        default_factory=list, description="属性列表"
    )
    relations: List[DetectedCharacterRelation] = Field(
...
class CharacterDetectionRequest(BaseModel):
    ...
    edition_id: int = Field(description="版本ID")
    range_selection: TextRangeSelection = Field(description="文本范围选择")
    config: CharacterDetectionConfig = Field(
        default_factory=CharacterDetectionConfig, description="检测配置"
    )
    work_title: str = Field(default="", description="作品标题")
    known_characters: List[str] = Field(
        default_factory=list, description="已知人物列表"
...
class CharacterDetectionResult(BaseModel):
    ...
    characters: List[DetectedCharacter] = Field(
        default_factory=list, description="检测到的人物列表"
    )
    metadata: Dict[str, Any] = Field(default_factory=dict, description="元数据")
...
class CharacterDetectionResponse(BaseModel):
    ...
    success: bool = Field(default=False, description="是否成功")
    task_id: Optional[str] = Field(default=None, description="任务ID")
    result: Optional[CharacterDetectionResult] = Field(
        default=None, description="检测结果"
    )
    message: str = Field(default="", description="消息")
    error: Optional[str] = Field(default=None, description="错误信息")
...
class ExtractedSetting(BaseModel):
    ...
    description: str = Field(default="", description="描述")
    attributes: List[ExtractedSettingAttribute] = Field(
        default_factory=list, description="属性列表"
    )
    relations: List[ExtractedSettingRelation] = Field(
        default_factory=list, description="关系列表"
    )
...
class SettingExtractionRequest(BaseModel):
    ...
    edition_id: int = Field(description="版本ID")
    range_selection: TextRangeSelection = Field(description="文本范围选择")
    config: SettingExtractionConfig = Field(
        default_factory=SettingExtractionConfig, description="提取配置"
    )
    work_title: str = Field(default="", description="作品标题")
...
class SettingExtractionResult(BaseModel):
    ...
    settings: List[ExtractedSetting] = Field(
        default_factory=list, description="提取的设定列表"
    )
    metadata: Dict[str, Any] = Field(default_factory=dict, description="元数据")
    raw_response: Optional[str] = Field(default=None, description="原始响应")
...
class SettingExtractionResponse(BaseModel):
    ...
    success: bool = Field(default=False, description="是否成功")
    task_id: Optional[str] = Field(default=None, description="任务ID")
    result: Optional[SettingExtractionResult] = Field(
        default=None, description="提取结果"
    )
    message: str = Field(default="", description="消息")
    error: Optional[str] = Field(default=None, description="错误信息")
...
class CharacterProfile(BaseModel):
    ...
    character: CharacterData = Field(
        default_factory=CharacterData, description="人物数据"
    )
    aliases: List[CharacterAliasData] = Field(
        default_factory=list, description="别名列表"
    )
    attributes: List[CharacterAttributeData] = Field(
        default_factory=list, description="属性列表"
    )
    arcs: List[CharacterArcData] = Field(default_factory=list, description="弧光列表")
    relations: List[CharacterRelationData] = Field(
        default_factory=list, description="关系列表"
    )
...
class AnalysisTaskData(BaseModel):
    ...
    target_node_ids: Optional[List[int]] = Field(
        default=None, description="目标节点ID列表"
    )
