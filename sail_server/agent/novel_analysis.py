...
from sail.llm.gateway import LLMExecutionConfig, TokenBudget
...
class NovelAnalysisAgent(BaseAgent):
    ...
    def validate_task(self, task: UnifiedAgentTask) -> ValidationResult:
        ...
        if sub_type and sub_type not in valid_sub_types:
            result.add_error(
                f"Invalid sub_type: {sub_type}. Must be one of {valid_sub_types}"
            )
        ...
        if not target_node_ids and not task.edition_id:
            result.add_warning(
                "No target_node_ids specified, will analyze entire edition"
            )
    ...
    def estimate_cost(self, task: UnifiedAgentTask) -> CostEstimate:
        ...
        from sail.llm.pricing import get_pricing
    ...
    async def execute(
        ...
        try:
            ...
            self._notify_progress(
                callback, 5, "preparing", "Preparing chapter chunks..."
            )
            ...
            # 3. 获取提示词模板
            sub_type = task.sub_type or task.config.get(
                "sub_type", TaskSubType.OUTLINE_EXTRACTION
            )
            template_id = task.prompt_template_id or TASK_TYPE_TEMPLATE_MAP.get(
                sub_type
            )
    ...
    async def _prepare_chunks(
        self, task: UnifiedAgentTask, context: AgentContext
    ) -> List[ChapterChunk]:
        """准备章节分块"""
        ...
        logger.info(
            f"[_prepare_chunks] Task {task.id}: range_mode={range_mode}, edition_id={task.edition_id}"
        )
        ...
        if task.edition_id:
            ...
            if range_mode == RangeSelectionMode.SINGLE_CHAPTER.value:
                ...
                logger.info(
                    f"[_prepare_chunks] Single chapter mode: chapter_index={chapter_index}"
                )
                ...
                logger.info(
                    f"[_prepare_chunks] Chapter range mode: start={start_index}, end={end_index}"
                )
                ...
                logger.info(
                    f"[_prepare_chunks] Multi chapter mode: indices={chapter_indices}"
                )
                if chapter_indices:
                    nodes = (
                        db.query(DocumentNode)
                ...
                start_index = range_selection.get("start_index")
                logger.info(
                    f"[_prepare_chunks] Current to end mode: start={start_index}"
                )
                ...
                logger.info(
                    f"[_prepare_chunks] Full edition mode: getting all chapters"
                )
                nodes = (
                    db.query(DocumentNode)
                    .filter(
    ...
    def _create_chunk(
        self, index: int, nodes: List[DocumentNode], content: str, tokens: int
    ) -> ChapterChunk:
        """创建分块"""
        if len(nodes) == 1:
            ...
            chapter_range = (
                f"{nodes[0].title or 'Chapter'} - {nodes[-1].title or 'Chapter'}"
            )
    ...
    async def _process_chunk(
        ...
        # 渲染提示词模板
        from sail.llm import get_template_manager
        ...
        provider = (
            task.llm_provider or config.get("llm_provider") or DEFAULT_LLM_PROVIDER
        )
        ...
        if provider.lower() == "moonshot":
            temperature = 1.0
            logger.info(
                f"[NovelAnalysis] Forced temperature to 1.0 for moonshot provider"
            )
        ...
        logger.info(
            f"[NovelAnalysis] LLM config: provider={provider}, model={model}, temperature={temperature}"
        )
    ...
    def _parse_llm_response(
        self, content: str, task_type: str, chunk: ChapterChunk
    ) -> List[AnalysisResult]:
        """解析 LLM 响应"""
        results = []
    ...
    def _parse_outline(
        self, parsed: Dict[str, Any], chunk: ChapterChunk
    ) -> List[AnalysisResult]:
        """解析大纲结果"""
        results = []
    ...
    def _compile_results(
        self, results: List[AnalysisResult], sub_type: str
    ) -> Dict[str, Any]:
        """编译所有结果"""
        # 按类型分组
        by_type = {}
    ...
    def _compile_outline_results(self, results: List[AnalysisResult]) -> Dict[str, Any]:
        """编译大纲提取结果为前端期望的格式"""
        nodes = []
        turning_points = []
        ...
        summaries = [
            r.data.get("summary", "")
            for r in results
            if r.result_type == "chunk_summary"
        ]
    ...
    async def _update_task_result(
        self, task: UnifiedAgentTask, context: AgentContext, result_data: Dict[str, Any]
    ):
        """更新任务结果"""
        try:
