...
class AsyncTaskManager:
    ...
    def start_task(self, task_id: int, coro) -> bool:
        ...
        # 任务完成后的回调
        async_task.add_done_callback(lambda t: self._on_task_done(task_id, t))
    ...
    def _on_task_done(self, task_id: int, async_task: asyncio.Task):
        ...
        try:
            ...
            self.task_results[task_id] = TaskRunResult(
                task_id=task_id, success=False, results_count=0, error_message=str(e)
            )
        finally:
            if task_id in self.running_tasks:
    ...
    def update_progress(self, task_id: int, progress: TaskProgress):
        ...
        self._notify_subscribers(
            task_id, {"type": "progress", "data": progress.to_dict()}
        )
...
class TaskExecutionController(Controller):
    ...
    async def create_execution_plan(
        self, db: Session, task_id: int, data: CreatePlanRequest
    ) -> Dict[str, Any]:
        """创建任务执行计划（预览）"""
        try:
    ...
    async def execute_task(
        self, db: Session, task_id: int, data: ExecuteTaskRequest
    ) -> Dict[str, Any]:
        """执行分析任务（同步模式，用于快速测试）"""
        try:
            mode = TaskExecutionMode(data.mode)
            ...
            if data.llm_provider == "mock":
                ...
                try:
                    ...
                    if not config.api_key:
                        logger.warning(
                            f"No API key found for provider {data.llm_provider} in environment, falling back to mock"
                        )
                        config = LLMConfig(
                            provider=LLMProvider.MOCK,
                            model="mock-model",
                    ...
                    logger.warning(
                        f"Failed to create config for provider {data.llm_provider}: {e}, falling back to mock"
                    )
                    config = LLMConfig(
                        provider=LLMProvider.MOCK,
                        model="mock-model",
    ...
    async def get_task_progress(self, db: Session, task_id: int) -> Dict[str, Any]:
        """获取任务进度"""
        task_manager = get_task_manager()
    ...
    async def stream_task_status(self, task_id: int) -> Stream:
        """SSE 端点 - 实时推送任务状态"""
        task_manager = get_task_manager()
        ...
        async def generate_events() -> AsyncIterator[bytes]:
            ...
            try:
                ...
                progress = task_manager.get_progress(task_id)
                if progress:
                    event = {"type": "progress", "data": progress.to_dict()}
    ...
    async def cancel_running_task(self, db: Session, task_id: int) -> Dict[str, Any]:
        """取消正在运行的任务"""
        task_manager = get_task_manager()
    ...
    async def import_external_result(
        self, db: Session, task_id: int, data: ImportResultRequest
    ) -> Dict[str, Any]:
        """导入外部 LLM 结果"""
        result = import_external_result(db, task_id, data.chunk_index, data.result_text)
...
class PromptTemplateController(Controller):
    ...
    @get("/")
    async def list_templates(self, task_type: Optional[str] = None) -> Dict[str, Any]:
        """获取模板列表"""
    ...
    async def preview_prompt(
        self, template_id: str, data: PreviewPromptRequest
    ) -> Dict[str, Any]:
        """预览渲染后的提示词"""
        manager = self._get_manager()
...
class PromptExportController(Controller):
    ...
    async def get_task_prompts(
        self, db: Session, task_id: int, format: str = "json"
    ) -> Dict[str, Any]:
        """获取任务的所有导出 Prompt"""
...
"""
            ...
            prompts.append(
                {
                    ...
                    "awaiting_result": r.result_data.get(
    ...
    async def download_prompts(
        self, db: Session, task_id: int, format: str = "markdown"
    ) -> Dict[str, Any]:
