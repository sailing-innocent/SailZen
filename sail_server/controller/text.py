...
class ImportController(Controller):
    ...
    async def append_chapters(
        ...
        if new_count == 0:
            raise NotFoundException(
                detail=f"Edition with ID {edition_id} not found or no chapters parsed"
            )
...
class AsyncImportController(Controller):
    ...
    async def create_import_task(
        ...
        return AsyncImportResponse(
            task_id=task.id, status=task.status, message="导入任务已创建，正在排队处理"
        )
    ...
    async def list_import_tasks(
        ...
        return ImportTaskListResponse(tasks=task_responses, total=total)
