...
class LoggingManager:
    ...
    def setup(self) -> logging.Logger:
        ...
        root_logger.info(
            f"Logging initialized. Level: {logging.getLevelName(self.log_level)}, "
    ...
    def _configure_third_party_loggers(self):
        ...
        # HTTP 客户端 - 默认关闭详细日志，只保留警告和错误
        logging.getLogger("httpx").setLevel(logging.WARNING)
        logging.getLogger("httpcore").setLevel(logging.WARNING)

        # 如果需要调试 HTTP 请求，可以临时开启
        # httpx_debug = os.getenv("HTTPX_DEBUG", "false").lower() == "true"
        # if httpx_debug:
