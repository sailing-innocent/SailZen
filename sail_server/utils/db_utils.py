    ...
    try:
        ...
        if seq_exists:
            ...
            if curr_val and curr_val <= max_id:
                ...
                new_val = max_id + 1
                db.execute(
                    text(f"SELECT setval('{sequence_name}', {new_val}, false)")
                )
                db.commit()
...
def fix_all_sequences(db: Session) -> dict:
    ...
    results = {}
    for table in tables:
        results[table] = fix_sequence(db, table)
    
    return results


def check_table_consistency(db: Session, table_name: str, id_column: str = "id") -> dict:
    """检查表的数据一致性
    ...
    try:
        ...
        min_id = db.execute(
            text(f"SELECT MIN({id_column}) FROM {table_name}")
        ).scalar()
