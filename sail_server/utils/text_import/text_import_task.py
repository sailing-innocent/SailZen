...
from sail_server.model.text import (
    create_work_impl, create_edition_impl, create_document_node_impl,
    WorkCreateRequest, EditionCreateRequest, DocumentNodeCreateRequest,
)
...
class TextImportTaskHandler:
    ...
    async def execute(self) -> ImportResult:
        ...
        try:
            ...
            result = ImportResult(
                ...
                chapter_count=len(self.parse_result.chapters) if self.parse_result else 0,
    ...
    async def _stage_preprocess(self):
        ...
        self._report_progress(
            100, 
            f"预处理完成，清理后共 {len(cleaned_text)} 字符"
        )
    ...
    async def _stage_parse(self):
        ...
        special_info = ", ".join(
            f"{k}: {v}" for k, v in special_chapters.items()
        ) if special_chapters else "无"
    ...
    def _report_progress(
        ...
        overall_progress = int((stage_start + stage_progress / 100 * stage_weight) * 100)
        ...
        logger.debug(
            f"Task {self.task_id} progress: {overall_progress}% - {message}"
        )
