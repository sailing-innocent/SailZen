...
class LLMRetryHandler:
    ...
    def _should_retry(self, error: Exception, attempt: int) -> tuple[bool, Optional[RateLimitInfo]]:
        """判断是否应该重试"""
    ...
    async def execute(
        ...
        on_retry: Optional[Callable[[int, float, Optional[RateLimitInfo]], None]] = None,
    ) -> RetryResult[T]:
        """执行带重试的操作
        ...
        while attempt <= self.config.max_retries:
            try:
                logger.debug(f"[RetryHandler] Attempt {attempt + 1}/{self.config.max_retries + 1}")
                ...
                if not should_retry or attempt >= self.config.max_retries:
                    logger.error(f"[RetryHandler] Giving up after {attempt + 1} attempts: {e}")
                    break
                
                # 计算等待时间
                delay = self._calculate_wait_time(attempt, is_rate_limit=rate_limit_info is not None)
    ...
    def get_rate_limit_stats(self) -> dict:
        ...
        recent_limits = [
            r for r in self._rate_limit_history
        ...
        return {
            ...
            "last_limit": self._rate_limit_history[-1].to_dict() if self._rate_limit_history else None,
