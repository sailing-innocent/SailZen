...
class TokenBudget:
    ...
    def check_budget(self, estimated_input: int, estimated_output: int, cost: float) -> tuple[bool, str]:
        """检查预算是否足够，返回 (是否通过, 消息)"""
        total_tokens = estimated_input + estimated_output
        
        if total_tokens > self.max_tokens:
            return False, f"Estimated tokens ({total_tokens}) exceeds budget ({self.max_tokens})"
        
        if cost > self.max_cost:
            return False, f"Estimated cost (${cost:.4f}) exceeds budget (${self.max_cost:.4f})"
        
        if cost > self.max_cost * self.warning_threshold:
            return True, f"Warning: Cost (${cost:.4f}) exceeds warning threshold ({self.warning_threshold * 100:.0f}%)"
...
class LLMCache:
    ...
    def _make_key(self, prompt: str, config: LLMExecutionConfig) -> str:
        """生成缓存键"""
        key_data = f"{prompt}:{config.model}:{config.temperature}:{config.system_prompt}"
        return hashlib.sha256(key_data.encode()).hexdigest()
    
    def get(self, prompt: str, config: LLMExecutionConfig) -> Optional[LLMExecutionResult]:
        """获取缓存结果"""
...
class LLMGateway:
    ...
    def register_provider(
        ...
        # 设置降级链
        self._fallback_chains[provider_name] = PricingRegistry.get_fallback_chain(config.model)
    ...
    async def execute(
        ...
        estimated_output_cost = (config.max_tokens or 1000) / 1000 * pricing.output_price
        
        # 检查预算
        budget_ok, budget_msg = budget.check_budget(
            int(estimated_input * 1000 / pricing.input_price) if pricing.input_price > 0 else 0,
            config.max_tokens or 1000,
            estimated_input + estimated_output_cost
        )
        
        if not budget_ok:
            raise ProviderError(f"Budget check failed: {budget_msg}", error_code="BUDGET_EXCEEDED")
    ...
    async def execute_with_fallback(
        ...
        for i, config in enumerate(configs):
            try:
                ...
                if i > 0:
                    logger.info(f"Fallback succeeded with {config.provider} after {i} retries")
    ...
    async def execute_auto_fallback(
        ...
        """
        # 构建降级配置列表
        fallback_models = self._fallback_chains.get(primary_config.provider, [primary_config.model])
