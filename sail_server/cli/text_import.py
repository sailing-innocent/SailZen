...
def parse_chapters_preview(content: str, pattern: Optional[str] = None) -> List[Tuple[str, str, int, int]]:
    """
...
def preview_chapters(chapters: List[Tuple[str, str, int, int]], 
                     show_count: int = 5,
                     content_preview_len: int = 100) -> None:
    """
    交互式预览章节切分结果
    """
    total = len(chapters)
...
def analyze_file(filepath: str, pattern: Optional[str] = None) -> Tuple[str, List[Tuple[str, str, int, int]], str]:
    """
    分析文件，返回内容和章节列表
    """
    # 检查文件是否存在
...
def confirm_import() -> bool:
    """确认是否导入"""
    while True:
        response = input(f"\n{colorize('是否确认导入？', Colors.YELLOW)} (y/n): ").strip().lower()
...
def do_import(content: str, chapters: List[Tuple[str, str, int, int]], 
              title: str, author: Optional[str], pattern: Optional[str],
              edition_name: Optional[str]) -> Tuple[int, int, int]:
    """
    执行导入操作
...
常用正则模式:
  ...
  方括号章节: ^【[^】]+】
    ...
    parser.add_argument("--import-text", dest="filepath", type=str, required=True,
                        help="要导入的 .txt 文件路径")
    ...
    mode_group.add_argument("--interactive", action="store_true",
                            help="交互式测试正则表达式")
    mode_group.add_argument("--confirm", action="store_true", default=True,
                            help="显示预览后确认导入（默认）")
    ...
    # 显示选项
    parser.add_argument("--show-count", type=int, default=5,
                        help="预览时显示的章节数量（默认: 5）")
    ...
    print(f"  环境: {colorize(env_mode.upper(), Colors.YELLOW if env_mode == 'dev' else Colors.RED)}")
