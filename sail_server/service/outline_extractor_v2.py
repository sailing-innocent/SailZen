...
class OutlineExtractorV2:
    ...
    def _create_default_llm_client(self) -> LLMClient:
        """创建默认 LLM 客户端"""
        from sail.llm.available_providers import DEFAULT_LLM_PROVIDER
    ...
    async def extract(
        ...
        if len(batches) == 1:
            ...
            if not validation["valid"]:
                logger.warning(
                    f"[V2 Extractor] Validation issues: {validation['issues']}"
                )
    ...
    async def _process_batch(
        ...
        if not retry_result.success:
            logger.error(
                f"[V2 Extractor] Batch {batch.batch_index} failed: {retry_result.error}"
            )
            # 返回空结果
    ...
    def _parse_response(
        self, content: str, batch: ExtractionBatch
    ) -> BatchExtractionResult:
        """解析 LLM 响应"""
        try:
            # 提取 JSON
            ...
            for node_data in data.get("outline_nodes", []):
                ...
                # 验证 chapter_index 范围
                chapter_index = anchor_data.get(
                    "chapter_index", batch.start_chapter_idx
                )
                if (
                    chapter_index < batch.start_chapter_idx
                    or chapter_index > batch.end_chapter_idx
                ):
                    logger.warning(
                ...
                node = ExtractedOutlineNodeV2(
                    id=node_data.get(
                        "id", f"batch{batch.batch_index}_node_{len(nodes)}"
                    ),
