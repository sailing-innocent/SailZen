...
class OutlineExtractor:
    ...
    def __init__(
        ...
        if llm_client is None:
            logger.info(
                f"[OutlineExtractor] Using default LLM config: {DEFAULT_LLM_PROVIDER}/{DEFAULT_LLM_MODEL}"
            )
    ...
    async def extract(
        ...
        logger.info(
            f"[Extractor] Starting extraction for edition {edition_id}, work_title='{work_title}'"
        )
        logger.info(
            f"[Extractor] Config: granularity={config.granularity}, outline_type={config.outline_type}, "
            f"extract_turning_points={config.extract_turning_points}, extract_characters={config.extract_characters}"
        )
        logger.info(
            f"[Extractor] Range selection: mode={range_selection.mode}, edition_id={range_selection.edition_id}, "
        ...
        # 1. 获取文本内容
        logger.info(
            f"[Extractor] Parsing text range selection: mode={range_selection.mode}"
        )
        ...
        if needs_chunking:
            logger.info(
                f"[Extractor] Using batch processing: {len(content_result.chapters)} chapters"
            )
    ...
    async def _extract_single_with_retry(
        ...
        """单块文本提取（带重试）"""
        logger.info(
            f"[_extract_single_with_retry] Called with config.llm_provider={config.llm_provider}, config.llm_model={config.llm_model}"
        )
                ...
                await progress_callback(
                    {
                        ...
                        "retry_delay": delay,
                        "rate_limit_info": rate_limit_info.to_dict()
                        if rate_limit_info
                        else None,
                    }
                )
        ...
        if not retry_result.success:
            ...
            error_info = ExtractionErrorInfo(
                ...
                rate_limit_info=retry_result.rate_limit_info.to_dict()
            ...
            logger.error(
                f"[Extractor] Extraction failed after {retry_result.attempts} attempts: {error_info}"
            )
    ...
    async def _extract_single(
        ...
        """单块文本提取"""
        logger.info(
            f"[_extract_single] Starting single-pass extraction for edition {edition_id}"
        )
        logger.info(
            f"[_extract_single] Config details: llm_provider={config.llm_provider}, llm_model={config.llm_model}, granularity={config.granularity}, outline_type={config.outline_type}"
        )
        ...
        try:
            ...
            logger.info(
                f"[_extract_single] Prompt rendered successfully, user_prompt length: {len(rendered.user_prompt)}"
            )
        except ValueError as e:
            # 如果 v2 模板不存在，使用 v1
            logger.warning(
                f"[_extract_single] Template {template_id} not found, falling back to v1: {e}"
            )
        ...
        if config.llm_provider or config.llm_model:
            ...
            try:
                ...
                logger.warning(
                    f"[_extract_single] Provider {provider} not available: {e}, falling back to {DEFAULT_LLM_PROVIDER}"
                )
            ...
            if provider.lower() == "moonshot":
                ...
                logger.info(
                    f"[_extract_single] Forced temperature to 1.0 for moonshot provider"
                )
            elif config.temperature is not None:
                llm_config.temperature = config.temperature
                logger.info(
                    f"[_extract_single] Set temperature to {config.temperature} for {provider}"
                )
            client = LLMClient(llm_config)
            logger.info(
                f"[_extract_single] Using custom LLM config: provider={provider}, model={llm_config.model}, temp={llm_config.temperature}"
            )
        else:
            client = self.llm_client
            logger.info(
                f"[_extract_single] Using default LLM client ({DEFAULT_LLM_PROVIDER}/{DEFAULT_LLM_MODEL})"
            )
        ...
        logger.info(
            f"[_extract_single] LLM response received, content length: {len(response.content)}"
        )
        ...
        logger.info(
            f"[_extract_single] Result parsed: {len(result.nodes)} nodes, {len(result.turning_points)} turning_points"
        )
        return result
    ...
    async def _extract_with_chunking(
        ...
        logger.info(
            f"[_extract_with_chunking] Total chapters: {total_chapters}, batch size: {CHAPTERS_PER_BATCH}, total_batches: {total_batches}"
        )
        ...
        if task_id and resume_from_checkpoint:
            ...
            if checkpoint:
                logger.info(
                    f"[_extract_with_chunking] Found checkpoint for task {task_id}, phase={checkpoint.phase}"
                )
                if checkpoint.phase == ExtractionPhase.COMPLETED.value:
                    # 如果已经完成，直接返回缓存的结果
                    logger.info(
                        f"[_extract_with_chunking] Checkpoint shows completed, recovering result"
                    )
        ...
        if checkpoint:
            ...
            if completed:
                ...
                logger.info(
                    f"[_extract_with_chunking] Recovered {len(all_nodes)} nodes from {len(completed)} completed batches"
                )
        ...
        for batch_idx in range(start_batch, total_batches):
            ...
            logger.info(
                f"[_extract_with_chunking] Processing batch {batch_idx + 1}/{total_batches}, chapters {start_idx + 1}-{end_idx}"
            )

            # 检查是否已完成
            if checkpoint and checkpoint.is_batch_completed(batch_idx):
                logger.info(
                    f"[_extract_with_chunking] Batch {batch_idx} already completed, skipping"
                )
                continue
            ...
            try:
                ...
                logger.info(
                    f"[_extract_with_chunking] Batch {batch_idx + 1} completed, nodes: {len(result.nodes)}"
                )
    ...
    def _recover_from_checkpoint(
        ...
        logger.info(
            f"[_recover_from_checkpoint] Recovered {len(nodes)} nodes, {len(turning_points)} turning points"
        )
    ...
    def _parse_extraction_result(
        ...
        """解析 LLM 输出结果"""
        logger.info(
            f"[_parse_extraction_result] Starting to parse response, content length: {len(response_content)}"
        )
        try:
            # 提取 JSON 部分
            json_str = self._extract_json(response_content)
            logger.info(
                f"[_parse_extraction_result] JSON extracted, length: {len(json_str)}"
            )
            ...
            logger.info(
                f"[_parse_extraction_result] Found {len(outline_nodes_data)} outline nodes in response"
            )
            for i, node_data in enumerate(outline_nodes_data):
                ...
                if i < 3:  # 只记录前3个节点的详细信息
                    logger.info(
                        f"[_parse_extraction_result] Node {i}: type={node.node_type}, title='{node.title[:50]}...', evidence_count={len(evidence_list)}"
                    )
            ...
            logger.info(
                f"[_parse_extraction_result] Found {len(turning_points_data)} turning points in response"
            )
            ...
            logger.error(
                f"[_parse_extraction_result] Failed to parse extraction result: {e}"
            )
            logger.error(
                f"[_parse_extraction_result] Raw response preview: {response_content[:500]}..."
            )
            return ServiceExtractionResult(
                nodes=[],
                turning_points=[],
            ...
            logger.error(
                f"[_parse_extraction_result] Unexpected error during parsing: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    def save_to_database(
        ...
        sorted_nodes = sorted(
            result.nodes, key=lambda n: (n.parent_id is not None, n.sort_index)
        )
    ...
    def _create_evidence_for_node(
        ...
        try:
            ...
            if node_id:
                ...
                logger.info(
                    f"Created evidence for outline_node {outline_node_id} linked to document_node {node_id}"
                )
            else:
                logger.warning(
                    f"Could not find suitable document node for edition {edition_id}, skipping evidence creation"
                )
