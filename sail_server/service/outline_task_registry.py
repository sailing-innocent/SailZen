...
from sail_server.service.persistent_checkpoint import (
    ...
    get_persistent_checkpoint_manager,
)
...
class OutlineTaskRegistry:
    ...
    async def execute_task(
        ...
        logger.info(
            f"[TaskRegistry] Task {task_id}: config_data keys={list(config_data.keys())}"
        )
        logger.info(
            f"[TaskRegistry] Task {task_id}: range_selection_data={range_selection_data}"
        )
        ...
        logger.info(
            f"[TaskRegistry] Task {task_id}: task.llm_provider={task.llm_provider}, using llm_provider={llm_provider}"
        )
        try:
            ...
            logger.warning(
                f"[TaskRegistry] Provider {llm_provider} not available: {e}, using default {DEFAULT_LLM_PROVIDER}"
            )
        ...
        config = OutlineExtractionConfig(
            ...
            extract_turning_points=extraction_config_data.get(
        ...
        logger.info(
            f"[TaskRegistry] Task {task_id}: Created OutlineExtractionConfig with llm_provider={config.llm_provider}, llm_model={config.llm_model}"
        )
    ...
    async def resume_task(
        ...
        if task.status not in ["paused", "failed"]:
            raise ValueError(
                f"Task {task_id} cannot be resumed (status: {task.status})"
            )
    ...
    def register_event_callback(
        self, callback: Callable[[int, str, Dict[str, Any]], None]
    ):
        """注册事件回调"""
