...
class TextCleaner:
    ...
    CHAPTER_REPEAT_PATTERN = re.compile(
        r"^(第[一二三四五六七八九十百千万零〇\d]+章.*?)$", re.MULTILINE
    )
    ...
    def _basic_clean(self, text: str) -> str:
        ...
        if text.startswith("\ufeff"):
            ...
            self.removed_items.append(
                {"type": "bom", "content": "UTF-8 BOM header", "position": 0}
            )
        ...
        for i, line in enumerate(lines):
            ...
            for pattern in self.AD_PATTERNS:
                if pattern.search(line):
                    is_ad = True
                    self.removed_items.append(
                        {"type": "ad", "content": line[:80], "line": i + 1}
                    )
                    break
    ...
    def _clean_symbols(self, text: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if self.SYMBOL_PATTERN.match(line):
                self.removed_items.append(
                    {"type": "symbols", "content": line[:30], "line": i + 1}
                )
                continue
            ...
            if self.SEPARATOR_PATTERN.match(line):
                self.removed_items.append(
                    {"type": "separator", "content": line[:30], "line": i + 1}
                )
                continue
    ...
    def get_sample_text(
        self, text: str, sample_size: int = 3000, num_samples: int = 3
    ) -> List[Tuple[int, str]]:
        """
        获取文本采样，用于AI分析
