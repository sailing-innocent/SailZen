...
def find_merged_chapters(chapters: List[Chapter]) -> List[Tuple[int, Chapter]]:
    ...
    for i, ch in enumerate(chapters):
        ...
        if ch.char_count > 50000:
            ...
            gap_before = (ch.chapter_num - prev_num) if (ch.chapter_num and prev_num) else 0
            gap_after = (next_num - ch.chapter_num) if (ch.chapter_num and next_num) else 0
...
def fix_merged_chapters(chapters: List[Chapter], merged_indices: List[int]) -> List[Chapter]:
    """修复被合并的章节"""
    fixed = []
...
def main():
    ...
    if merged:
        ...
        for idx, ch in merged[:5]:
            ...
            next_num = chapters[idx+1].chapter_num if idx < len(chapters)-1 else None
            print()
        ...
        if args.yes:
            ...
            confirm = input(colorize("是否自动修复这些章节? (y/n): ", Colors.YELLOW)).strip().lower()
    ...
    if args.yes:
        ...
        final_confirm = input(colorize(f"\n确认导入 '{args.title}' 到数据库? (y/n): ", Colors.YELLOW)).strip().lower()
    
    if final_confirm in ['y', 'yes', '是']:
        import_to_database(chapters, args.title, args.author, env_mode)
    else:
