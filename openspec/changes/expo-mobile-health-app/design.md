...
- 如扩展至多用户，可考虑 JWT Token 或 API Key 机制
- 移动端支持生物识别登录（指纹/面部识别）

### 7. 服务器地址配置
**Decision**: 使用环境配置文件管理 API 地址，与 Web 端策略一致
...
// .env.production
API_BASE_URL=https://api.sailzen.example.com
```

### 8. 数据保留策略
**Decision**: 本地数据长期保留，无需主动清理

**Rationale**:
- 健康数据具有长期价值，用户可能需要查看历史记录
- SQLite 存储容量充足，数据量不会成为问题
- 以服务器数据为准，本地仅作缓存和离线使用
- 定期同步确保数据安全
...
### 9. 图表设计策略
**Decision**: 移动端图表独立设计，优先适配移动场景

**Rationale**:
- 屏幕尺寸和交互方式与 Web 端差异大
- 移动端更注重信息密度和操作便捷性
- 遵循移动端设计规范（Material Design 3）
- Web 端可借鉴移动端验证后的 UX 模式

**Mobile-First Charts**:
- 使用 react-native-gifted-charts 替代 Web 端图表库
- 优化触摸交互和手势支持
- 适配小屏幕的简洁信息展示
- 支持横竖屏切换

### 10. 推送通知
**Decision**: 集成 Expo Notifications 实现健康记录提醒

**Rationale**:
- 提醒用户记录体重、运动是移动端的核心价值
- 培养用户健康习惯，提高应用粘性
- Expo Notifications 提供跨平台支持（为未来 iOS 预留）
- 支持本地通知（无需服务器推送）
...
| 推送通知权限被拒绝 | 用户无法收到提醒 | 友好的权限引导，提供设置入口，尊重用户选择 |
| 无认证机制的安全风险 | 个人数据暴露 | 第一阶段限制在内网使用，后续添加 API Key 或 JWT |
...
所有 Open Questions 已得到解答并整合到上面的设计决策中：

1. **认证机制**: 第一阶段暂不实现，API 直接访问
2. **服务器地址**: 使用环境配置文件管理，与 Web 端策略一致
3. **数据保留策略**: 本地数据长期保留，无需主动清理
4. **图表精度**: 移动端独立设计，优先适配移动场景
5. **推送通知**: 集成 Expo Notifications 实现健康记录提醒
