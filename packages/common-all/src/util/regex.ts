...
/** Returns true if this is a non-sail uri, false if it is sail://, undefined if it's not a URI */
export const containsNonSailUri = (uri: string): boolean | undefined => {
  const groups = uriRegex.exec(uri)?.groups;
  if (_.isUndefined(groups) || _.isUndefined(groups.scheme)) return undefined;
  if (groups.scheme === "sail") return false;
  return true;
};
