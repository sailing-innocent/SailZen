...
import type { ISailError } from "./error";
import { SailError, assertUnreachable } from "./error";
import { ERROR_STATUS } from "./constants";

export type SidebarResult<T> = Result<T, ISailError>;
...
export function parseSidebarConfig(
  input: unknown
): Result<SidebarConfig, ISailError> {
          ...
          return err(
            new SailError({
              message: `Invalid sidebar item type in \`${JSON.stringify(
                maybeSidebarItem
              )}\`. Valid are \`${noteLiteral.value}\`, \`${categoryLiteral.value
  ...
  return err(new SailError({ message: "Sidebar object is not an array" }));
}
