 ...
 */
export type SailConfigEntry<T = any> = {
  value?: T;
  label: string;
  desc: string;
};
 ...
 * maps config properties to their respective SailConfigEntry
 */
export type SailConfigEntryCollection<T> = {
  [Property in keyof T]-?:
  | SailConfigEntryCollection<any>
  | Record<any, SailConfigEntry>
  | SailConfigEntry
  | ((...args: any[]) => SailConfigEntry);
};
