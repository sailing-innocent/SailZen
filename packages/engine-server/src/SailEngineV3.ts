...
export class SailEngineV3 extends EngineV3Base implements DEngine {
  ...
  async init(): Promise<DEngineInitResp> {
    ...
    try {
      ...
      switch (_.size(allErrors)) {
        ...
        case 1: {
          error = new SailError(allErrors[0]);
          break;
        }
        default:
          error = new SailCompositeError(allErrors);
      }
      this.logger.info({
        ctx: "init:ext",
        error,
    ...
    if (!oldNote) {
      return {
        error: new SailError({
          ...
          message:
            `Unable to rename note "${oldLoc.fname
    ...
    if (
      ...
      if (out.error) {
        return {
          error: new SailError({
            message:
              `Unable to delete note "${oldNote.fname
  ...
  async info(): Promise<EngineInfoResp> {
    ...
    if (!version) {
      return {
        error: SailError.createPlainError({
          message: "Unable to read Sail version",
        }),
      };
    }
    return {
  ...
  private async initNotes(
    ...
    return {
      ...
      error:
        errors.length === 0 ? undefined : new SailCompositeError(errors),
    };
  }
