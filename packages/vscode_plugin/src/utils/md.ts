...
export async function getReferenceAtPosition({
  ...
  if (!rangeWithLink) {
    const { enableZDocTags, enableHashTags } = ConfigUtils.getWorkspace(
    ...
    if (enableZDocTags) {
      ...
      const rangeForZDocTag = document.getWordRangeAtPosition(
        position,
        ZDOCTAG_REGEX_LOOSE
      );
      if (rangeForZDocTag) {
        const docText = document.getText(rangeForZDocTag);
        ZDOCTAG_REGEX_LOOSE.lastIndex = 0;
        const match = ZDOCTAG_REGEX_LOOSE.exec(docText);
        if (_.isNull(match)) return null;

        return {
          range: rangeForZDocTag,
          label: match[0],
          ref: `${ZDOCS_HIERARCHY}${match.groups!.zdocTagContents}`,
          refText: docText,
          refType: "zdoctag",
        };
      }
