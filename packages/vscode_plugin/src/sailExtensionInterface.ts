...
export type SailWorkspaceSettings = Partial<{
  "sail.dailyJournalDomain": string;
  "sail.defaultJournalName": string;
  "sail.defaultJournalDateFormat": string;
  "sail.defaultJournalAddBehavior": string;
  "sail.defaultScratchName": string;
  "sail.defaultScratchDateFormat": string;
  "sail.defaultScratchAddBehavior": string;
  "sail.copyNoteUrlRoot": string;
  "sail.linkSelectAutoTitleBehavior": string;
  "sail.defaultLookupCreateBehavior": string;
  "sail.defaultTimestampDecorationFormat": string;
  "sail.rootDir": string;
  "sail.sailDir": string;
  "sail.logLevel": string;
  "sail.trace.server": string;
  "sail.serverPort": string;
}>;

/**
 * This is THE interface of SailExtension.
 *
 * Going forward we should NOT be using SailExtension object directly
 ...
 * */
export interface ISailExtension {
  port?: number;
  /**
  ...
  /**
   * This will deactivate the entire Sail Extension. Takes care of disposing of all resources that Sail has created
   */
  ...
  /**
   * Checks if a Sail workspace is currently active and that the current {@link fpath} is a valid Sail Note
   * @param fpath: full path to current file
   */
  isActiveAndIsSailNote(fpath: string): Promise<boolean>;
