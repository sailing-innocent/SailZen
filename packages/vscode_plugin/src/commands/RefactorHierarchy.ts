...
import { RenameNoteOutput, RenameNoteInternalCommand } from "./RenameNoteInternal";
import {
  MultiSelectBtn,
  Selection2ItemsBtn,
...
export type CommandOutput = RenameNoteOutput & {
...
> {
  ...
  async promptMatchText() {
    ...
    const value = editor?.document
      ? (await WSUtils.instance().getNoteFromDocument(editor.document))?.fname
      : "";
  ...
  async runOperations(opts: {
    operations: RenameOperation[];
    renameCmd: RenameNoteInternalCommand;
  }) {
