import {
  ...
  NoteUtils,
  VaultUtils,
  ZDOCS_HIERARCHY, TAGS_HIERARCHY, TAGS_PREFIX,
  ZDOCS_TAG_PREFIX
...
> {
  ...
  prepareBrokenLinkConvertOptions(reference: getReferenceAtPositionResp) {
    ...
    const hierarchyOption: QuickPickItem = {
      label: "Hierarchy",
      description:
        reference.refType === "zdoctag" || reference.refType === "hashtag"
          ? reference.ref
          : parsedLink?.value,
  ...
  async prepareValidLinkOperation(reference: getReferenceAtPositionResp) {
    ...
    switch (refType) {
      ...
      case "wiki": {
        let tagType;
        if (ref.startsWith(ZDOCS_HIERARCHY)) {
          tagType = "zdoctag";
        } else if (ref.startsWith(TAGS_HIERARCHY)) {
          tagType = "hashtag";
        }
        ...
        if (shouldProceed) {
          ...
          let text = label;
          if (tagType == "hashtag") {
            text = `${TAGS_PREFIX}${label}`;
          }
          else if (tagType == "zdoctag") {
            text = `${ZDOCS_TAG_PREFIX}${label}`;
          }
