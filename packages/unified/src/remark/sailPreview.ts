...
import { SailASTTypes, HashTag, ZDocTag, WikiLinkNoteV4 } from "../types";
...
function modifyWikilinkValueToCommandUri({
  ...
  node.value = `command:sail.gotoNote?${encodedArgs}`;
}
...
function modifyTagValueToCommandUri({
  ...
  (node as unknown as Link).url = `command:sail.gotoNote?${encodedArgs}`;
...
export function sailHoverPreview(
  this: Processor,
  _opts?: PluginOpts
): Transformer {
  const proc = this;
  function transformer(tree: Node, _file: VFile) {
    visit(
      tree,
      [
        SailASTTypes.FRONTMATTER,
        SailASTTypes.IMAGE,
        SailASTTypes.EXTENDED_IMAGE,
...
export { PluginOpts as SailPreviewOpts };
