...
type PluginOpts = NoteRefsOptsV2 & {
  ...
  insertTitle?: boolean;
  /**
   * Don't publish pages that are dis-allowd by sail.yml
   */
...
type SailUnifiedHandlerMatchOpts = {
  pOpts: ProcOptsV5;
  pData: ProcDataFullOptsV5;
};

type SailUnifiedHandlerHandleOpts<T = any> = {
  proc: Processor;
  parent: Node;
  cOpts?: T;
};

type SailUnifiedHandlerNextAction = undefined | number;
...
class NodeUrlHandler {
  ...
  static match(node: Node | any, { pData }: SailUnifiedHandlerMatchOpts) {
    return (
      (node.type === SailASTTypes.IMAGE ||
        node.type === SailASTTypes.EXTENDED_IMAGE ||
        node.type === SailASTTypes.LINK) &&
      pData.dest === SailASTDest.HTML
    );
  }

  static handle(
    node: Image | Link,
    { proc, cOpts }: SailUnifiedHandlerHandleOpts<PluginOpts>
  ): { node: Image | Link; nextAction?: SailUnifiedHandlerNextAction } {
  ...
  function transformer(tree: Node, _file: VFile) {
    ...
    if (mode !== ProcMode.IMPORT && !insideNoteRef && root.children) {
      if (!fname || !vault) {
        // TODO: tmp
        throw new SailError({
          message: `sailPub - no fname or vault for node: ${JSON.stringify(
            tree
          )}`,
        });
      }
      ...
      if (insertTitle) {
        ...
        root.children.splice(
          idx,
          0,
          u(SailASTTypes.HEADING, { depth: 1 as const }, [u("text", note.title)]) as any
        );
      }
    }
    visitParents(tree, (node: Node, ancestors: Node[]) => {
      ...
      if (node.type === SailASTTypes.HASHTAG) {
      ...
      if (node.type === SailASTTypes.ZDOCTAG) {
        const zdocTag = node as ZDocTag;
        const parentIndex = _.findIndex(parent.children, node);
        if (parentIndex === -1) return;
        // Convert user tags to regular links for rendering
        // but not if they are inside of a link, otherwise they break link rendering.
        if (!ancestors.some((node) => RemarkUtils.isLink(node))) {
          node = zdocTag2WikiLinkNoteV4(zdocTag);
        } else {
          node = text(zdocTag.value);
        }
        parent.children[parentIndex] = node;
      }
      if (node.type === SailASTTypes.WIKI_LINK) {
        ...
        if (mode !== ProcMode.IMPORT) {
          ...
          if (!note) {
            error = new SailError({ message: `no note found. ${value}` });
          }
        }
        ...
        if (value === "403") {
          ...
          _node.data = {
            ...
            hProperties: {
              title: "Private",
              href: "https://wiki.sail.so/notes/hfyvYGJZQiUwQaaxQO27q.html",
              target: "_blank",
              class: "private",
            },
            hChildren: [
      ...
      if (node.type === SailASTTypes.BLOCK_ANCHOR) {
        // no transform
...
export { plugin as sailPub };
export { PluginOpts as SailPubOpts };
