...
export class LinkUtils {
  ...
  static renderNoteLink({
    ...
    switch (dest) {
      case DendronASTDest.MD_DENDRON: {
        ...
        if (this.isZDocTagLink(link.from)) {
          return ZDOCS_TAG_PREFIX + link.from.alias + ZDOCS_TAG_SUFFIX;
        }
  ...
  static isZDocTagLink(link: DNoteLoc): link is DNoteLoc & { alias: string } {
    return (
      link.alias !== undefined &&
      // link.alias.startsWith("@") &&
