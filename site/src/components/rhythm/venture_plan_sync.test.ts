...
describe('syncPlanAfterVentureChange', () => {
  ...
  test('calls api_plan_day once for today by default', async () => {
    const results = await syncPlanAfterVentureChange('2026-10-26')
  ...
  test('calls api_plan_day for N consecutive days', async () => {
    const results = await syncPlanAfterVentureChange('2026-10-26', { days: 3 })
  ...
  test('passes force=false through to api_plan_day', async () => {
    await syncPlanAfterVentureChange('2026-10-26', { days: 2, force: false })
    expect(api_plan_day).toHaveBeenCalledTimes(2)
    expect(api_plan_day).toHaveBeenCalledWith(expect.any(Date), { preserve_done: true, force: false })
  })
  test('returns per-day plan results', async () => {
    api_plan_day
      .mockResolvedValueOnce({ date: '2026-10-26', blocks: [1] })
      .mockResolvedValueOnce({ date: '2026-10-27', blocks: [2, 3] })
    const results = await syncPlanAfterVentureChange('2026-10-26', { days: 2 })
    expect(results[0].blocks).toEqual([1])
    expect(results[1].blocks).toEqual([2, 3])
  })
})
