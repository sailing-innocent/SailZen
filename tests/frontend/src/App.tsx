...
import { GitBranch, Play, History, Square, RefreshCw, Users, Workflow } from 'lucide-react'
import { useAppStore } from './store/useAppStore'
import DAGCanvas from './components/nodes/DAGCanvas'
...
type ViewMode = 'pipelines' | 'agents'
  ...
  return (
    <div className="h-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden">
      <header className="flex items-center gap-3 px-5 py-3 border-b border-slate-800 bg-slate-900 flex-shrink-0">
        ...
        <h1 className="font-bold text-lg tracking-tight">CubeClaw</h1>

        {/* View Mode Toggle */}
        <div className="flex items-center gap-1 ml-4 p-1 bg-slate-800 rounded-lg">
          <button
            onClick={() => setViewMode('agents')}
            className={cn(
              'px-3 py-1.5 text-xs font-medium rounded-md transition-colors flex items-center gap-1.5',
              viewMode === 'agents' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-200'
            )}
          >
          ...
          <button
            ...
            className={cn(
              'px-3 py-1.5 text-xs font-medium rounded-md transition-colors flex items-center gap-1.5',
              viewMode === 'pipelines' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-200'
            )}
          >
