...
```bash
# 使用 AI 分析并导入
python scripts/import_with_ai.py novel.txt --title "小说标题" --author "作者名"

# 仅预览分析结果（不导入）
python scripts/import_with_ai.py novel.txt --title "小说标题" --preview

# 使用规则解析（不使用 AI）
python scripts/import_with_ai.py novel.txt --title "小说标题" --no-ai

# 跳过确认直接导入
python scripts/import_with_ai.py novel.txt --title "小说标题" --yes
```
