...
class AIChapterParser:
    ...
    def __init__(
        self, llm_client=None, sample_size: int = 3000, anomaly_threshold: float = 3.0
    ):
        """
        初始化解析器
    ...
    def _learn_pattern_from_samples(
        ...
        for chapter in chapters:
            if chapter.char_count > avg + self.anomaly_threshold * std:
                ...
                chapter.warnings.append(
                    f"超长章节 ({chapter.char_count:,} 字，平均 {avg:,.0f} 字)"
                )
...
"""
    ...
    def _classify_chapter_type(
        self, title: str, special_types: Dict[str, str] = None
    ) -> ChapterType:
        """分类章节类型"""
